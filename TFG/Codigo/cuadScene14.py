...
Numero_experimentos= 500
TamanoPaso = 100
cuarto= Numero_experimentos // 4
Division= Numero_experimentos // TamanoPaso
Tamano= 0.075// Division
#550000
...
def obtener_nombre_archivo(nombre_base):
    ...
    while os.path.exists(nombre_archivo + '.pickle'):
        numero += 1
...
def crecer(sim,client,ext,pos):
    sim.setJointTargetPosition(ext[0],pos)
    sim.setJointTargetPosition(ext[1],pos)
    sim.setJointTargetPosition(ext[2],pos)
    sim.setJointTargetPosition(ext[3],pos)

    sim.setJointPosition(ext[0],pos)
    sim.setJointPosition(ext[1],pos)
    sim.setJointPosition(ext[2],pos)
    sim.setJointPosition(ext[3],pos)
...
#     
    ...
    i=0
    while episodios < Numero_experimentos:   
        print(".....................................................")
        ...
        for pasos in range(TamanoPaso):
            ...
            if refuerzo >=0:
                rewards[episodios] = refuerzo
                #print(rewards[experimento],"valor del refuerzo aqui")


            numpy.save('matrizQe.npy', Matrizq)
            numpy.save('Lista_movimientose.npy', Lista_movimientos)
            numpy.save('Lista_estadose.npy', Lista_estados)
            numpy.save('rewardse.npy', rewards)
                

            print("Esto es el episodio dentro del paso ", pasos)
            print("Esta es el episodio en global", episodios)
        ...
        #crecer(sim,client,ext,i*Tamano)
        #i=i+1

        if (episodios >= cuarto):
            if (episodios <= 2*cuarto):
                crecer(sim,client,ext,0.025)
            if (episodios >= 2*cuarto):  
                if (episodios <= 3*cuarto):
                    crecer(sim,client,ext,0.05)
                if (episodios >= 3*cuarto):
                    crecer(sim,client,ext,0.075)
    ...
    sim.stopSimulation()
    numpy.save('seacabo.npy', Lista_estados)
